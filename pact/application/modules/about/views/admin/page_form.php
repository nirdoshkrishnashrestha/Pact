...
$(function(){
	$('#home_image').uploadify({
		formData     	: {
					...
					'targetFolder' : 'acp/uploads/about',
	...
	/*	//Default as provided by site: ITS ALWAYS SAME, THE ORDER NEEDS TO REMAIN SAME, NO MATTER THE NAME OF VARIABLE!
		onUploadSuccess : function(file, data, response) {
  		...
  		onUploadSuccess : function(file, data, response) {	//alert(file.name);
					...
					$('.imagethumbs-form').prepend('<div class="imagethumb-form additional-file-input" id="add-image1" > <a class="close-msg" title="Delete" id="deleteImg">Delete</a> <a href="#" title="'+data+'" class="img-wrap"><img src="'+"<?php echo base_url()?>"+'assets/createThumb/create_thumb.php?src='+imagePath+'uploads/about/'+data+'&w=150&h=150" alt="'+data+'" /></a></div>');
					//alert($('#fileList').val());					
...
$(function(){
	$('a#deleteImg').live('click',function(){		
		...
		delete_image(_img);
		$.post("<?php echo admin_url("about/delete_image");?>",{imgName:_img},
		function(data){
...
<tr>
  <td valign="top"><strong>Order:</strong><span class="input-info">Arrange the order</span></td>
...
?>
      <div class="imagethumb-form additional-file-input" id="add-image1">
            ...
            <a href="#" title="<?php echo $i;?>" class="img-wrap">
                <img src="<?php echo base_url()?>assets/createThumb/create_thumb.php?src=<?php echo ROOT;?>uploads/about/<?php echo $i;?>&w=150&h=150" />
            </a>  
        </div>
<?php endforeach;} }?>
    </div>
