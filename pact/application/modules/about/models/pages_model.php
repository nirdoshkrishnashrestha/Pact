...
class Pages_model extends CI_Model
{
	function __construct(){
			parent::__construct();			
			$table_name = 'tbl_about';
	}
	
	function totalPages($where=NULL)
	{	
		$sql = "SELECT * FROM `tbl_about` AS p LEFT JOIN `tbl_static_pages_category` AS c ON p.pc_id= c.id";
	...
	{
		$sql = "SELECT * FROM `tbl_static_pages_category` AS c LEFT JOIN `tbl_about` AS p ON p.pc_id= c.id";
		if($where)
		$sql.= ' where'." ".$where;
