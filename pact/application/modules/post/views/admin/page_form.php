...
$(function(){
	$('#home_image').uploadify({
		formData     	: {
					...
					'targetFolder' : 'pact/uploads/post',
	...
	/*	//Default as provided by site: ITS ALWAYS SAME, THE ORDER NEEDS TO REMAIN SAME, NO MATTER THE NAME OF VARIABLE!
		onUploadSuccess : function(file, data, response) {
  		...
  		onUploadSuccess : function(file, data, response) {	//alert(file.name);
					...
					$('.imagethumbs-form').prepend('<div class="imagethumb-form additional-file-input" id="add-image1" > <a class="close-msg" title="Delete" id="deleteImg">Delete</a> <a href="#" title="'+data+'" class="img-wrap"><img src="'+"<?php echo base_url()?>"+'assets/createThumb/create_thumb.php?src='+imagePath+'uploads/post/'+data+'&w=150&h=150" alt="'+data+'" /></a></div>');
					//alert($('#fileList').val());					
...
$(function(){
	$('a#deleteImg').live('click',function(){		
		...
		delete_image(_img);
		$.post("<?php echo admin_url("post/delete_image");?>",{imgName:_img},
		function(data){
...
<tr>
  <td ><strong>Select catagory</strong></td>
  <td>
  <?php
   ...
   $sqls1 = "select * from tbl_catagories where id = ".$details->cat_id. ' limit 1';  /// this will get the cat_id of tbl_post
   $query_cat1 = $this->db->query($sqls1)->result();}
   ?>
  <select name="cat_id" class="selectType" require>
  <?php 
 	 foreach($query_cat1 as $cat_edit){  
	 //// this is done because we need to use foreach and this option is for default option. Vary in edit and add new.  
   ?>
  <option value="<?php if(isset($isEdit)){ echo $cat_edit->id;} ?>">
  <?php if(isset($isEdit)){ echo $cat_edit->page_title;}  ///// there must be value in cat_id?>
   </option>
   <?php } if($isEdit){ ?>
   
  <?php foreach($query_cat0 as $cat0){  // if edit then display catagory except default i.e. existing value if cat_id  ?>
...
<tr>
  <td valign="top"><strong>Order:</strong><span class="input-info">Arrange the order of post.</span></td>
...
?>
      <div class="imagethumb-form additional-file-input" id="add-image1">
            ...
            <a href="#" title="<?php echo $i;?>" class="img-wrap">
                <img src="<?php echo base_url()?>assets/createThumb/create_thumb.php?src=<?php echo ROOT;?>uploads/post/<?php echo $i;?>&w=150&h=150" />
            </a>  
        </div>
<?php endforeach;} }?>
    </div>
