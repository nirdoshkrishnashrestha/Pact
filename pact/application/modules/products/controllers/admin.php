<?php
	class Admin extends CI_Controller{
		...
		function index()
		{
			...
			create_pagination(base_url().'admin/products/index/?pagi=1',$data['total_pages'], 4, $this->settings->item('rows_per_page'));																		
		...
		{ //echo "<pre>";var_dump($_POST);die;
			...
			$data['details'] = $this->general_db_model->getById('products', 'id', $id);	
			$data['get_page'] = $this->general_db_model->get_all_menu_pages('static_pages', 'pc_id', '5');
		...
		function add_edit($id=NULL)
		{
			...
			{ 
				...
				$this->general_db_model->update('products', $post, 'id = '.$id);
			...
			else
			{
				...
				$home_image = $this->input->post('fileList');
				$post['home_image'] = $home_image;
				$this->general_db_model->insert('products', $post);
		...
		function delete($id)
		{
			$this->general_db_model->del_img($id,'products','banners');
		...
		{ 
			...
			if($status == 'Publish')
				...
				$data['status'] = 'Publish';
			$this->general_db_model->update('products', $data, 'id = '.$id);
