<?php
	class Admin extends CI_Controller{
		...
		function index()
		{
			...
			create_pagination(base_url().'admin/download/index/?pagi=1',$data['total_pages'], 4, $this->settings->item('rows_per_page'));																		
		...
		function edit($id)
		{
			...
			$data['details'] = $this->general_db_model->getById('download', 'id', $id);	
			$data['get_page'] = $this->general_db_model->get_all_menu_pages('static_pages', 'pc_id', '5');
		...
		function add_edit($id=NULL)
		{
			if($id)
			{
				...
				$this->general_db_model->update('download', $post, 'id = '.$id);
			...
			else
			{
				...
				$home_image = $this->input->post('fileList');
				$post['home_image'] = $home_image;
				$this->general_db_model->insert('download', $post);
		...
		function delete($id)
		{
			$this->general_db_model->del_img($id,'download','banners');
			$this->general_db_model->delete('download', 'id = '.$id);			
		...
		{ 
			...
			if($status == 'Publish')
				...
				$data['status'] = 'Publish';
			$this->general_db_model->update('download', $data, 'id = '.$id);
