...
$(function(){
	$('#home_image').uploadify({
		formData     	: {
					...
					'targetFolder' : 'Slink/uploads/download',
	...
	/*	//Default as provided by site: ITS ALWAYS SAME, THE ORDER NEEDS TO REMAIN SAME, NO MATTER THE NAME OF VARIABLE!
		onUploadSuccess : function(file, data, response) {
  		...
  		onUploadSuccess : function(file, data, response) {	//alert(file.name);
					...
					$('.imagethumbs-form').prepend('<div class="imagethumb-form additional-file-input" id="add-image1" > <a class="close-msg" title="Delete" id="deleteImg">Delete</a> <a href="#" title="'+data+'" class="img-wrap"><img src="'+"<?php echo base_url()?>"+'assets/createThumb/create_thumb.php?src='+imagePath+'uploads/download/'+data+'&w=150&h=150" alt="'+data+'" /></a></div>');
					//alert($('#fileList').val());					
...
$(function(){
	$('a#deleteImg').live('click',function(){		
		...
		delete_image(_img);
		$.post("<?php echo admin_url("download/delete_image");?>",{imgName:_img},
		function(data){
...
<tr>
  <td valign="top"><strong> File:</strong><span class="input-info">Choose file (Only one).</span></td>
...
?>
      <div class="imagethumb-form additional-file-input" id="add-image1">
            ...
            <?php if($ext == "pdf"){ ?>
            <img src="<?php echo base_url()?>images/pdf.jpg" width="50px" height="50px" />
            <?php }else if($ext == "doc" or $ext == "docx"){ ?>    
            <img src="<?php echo base_url()?>images/doc.jpg" width="50px" height="50px"/>
            <?php }else{ ?>
            <img src="<?php echo base_url()?>images/other.jpg" width="50px" height="50px" />
            <?php } ?>
            </a>  
        </div>
<?php endforeach;} }?>
